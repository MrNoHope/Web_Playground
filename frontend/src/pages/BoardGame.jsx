...
const BoardGame = () => {
  ...
  const handleMenuControl = (action) => {
    if (action === 'LEFT') {
      setCursor((prev) => (prev > 0 ? prev - 1 : GAMES_LIST.length - 1));
    } else if (action === 'RIGHT') {
      setCursor((prev) => (prev < GAMES_LIST.length - 1 ? prev + 1 : 0));
    } else if (action === 'ENTER') {
      setSelectedGame(GAMES_LIST[cursor]);
    } else if (action === 'HINT') {
      alert(`Chọn game bằng Left/Right, nhấn ENTER để vào chơi.`);
    }
  };
  ...
  return (
    <div className="board-wrapper">
      ...
      <h3 style={{ color: GAMES_LIST[cursor].color, height: '30px', marginTop: '0' }}>
        {GAMES_LIST[cursor].name}
      </h3>
        ...
        {GAMES_LIST.map((game, index) => (
          <div
            ...
            style={{ 
              ...
              height: '45px', 
              borderRadius: '50%',
              border: cursor === index ? '4px solid var(--text-primary)' : '4px solid transparent',
              boxSizing: 'border-box',
              opacity: cursor === index ? 1 : 0.6,
              transform: cursor === index ? 'scale(1.1)' : 'scale(1)',
              transition: 'all 0.2s'
            }}
          />
        ))}
